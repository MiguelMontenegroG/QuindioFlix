...
export default function PodcastsPage() {
  ...
  useEffect(() => {
    async function cargar() {
      try {
        const response = await contenidoAPI.obtenerTodos({ categoria: 'Podcast', por_pagina: 50 })
        if (response?.data) setContenido(response.data)
  } catch { setContenido(mockContenido.filter((c) => c.categoria === 'Podcast')) }
      if (perfilActivo) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-24 pb-12 px-4 md:px-12">
        <h1 className="text-4xl font-bold text-foreground mb-8">Podcasts</h1>
        {filtrado.length > 0 ? <ContentCarousel title="Podcasts" contenido={filtrado} favoritos={favoritos} onFavoriteToggle={handleFavoriteToggle} /> : <p className="text-muted-foreground">No hay podcasts disponibles.</p>}
      </main>
    </div>
  )
}
