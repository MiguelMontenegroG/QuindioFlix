...
export default function PeliculasPage() {
    ...
    async function cargar() {
      ...
      if (perfilActivo) {
        try {
          ...
          setFavoritos(favs.map((f) => f.id_contenido))
        } catch {}
      }
      setIsLoading(false)
    }
  ...
  const handleFavoriteToggle = async (id: number) => {
    ...
    try {
      if (favoritos.includes(id)) {
        await favoritosAPI.eliminar(perfilActivo.id, id)
        setFavoritos((prev) => prev.filter((fav) => fav !== id))
      } else {
        await favoritosAPI.agregar(perfilActivo.id, id)
  ...
  const listaPerfiles: Perfil[] = perfiles.length > 0 ? perfiles : mockPerfiles
  const filtrado = perfilActivo?.es_infantil
    ? contenido.filter((c) => ['TP', '+7', '+13'].includes(c.clasificacion_edad))
    : contenido
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-24 pb-12 px-4 md:px-12">
        <h1 className="text-4xl font-bold text-foreground mb-8">Peliculas</h1>
        {filtrado.length > 0 ? (
          <ContentCarousel title="Peliculas" contenido={filtrado} favoritos={favoritos} onFavoriteToggle={handleFavoriteToggle} />
        ) : (
          <p className="text-muted-foreground">No hay peliculas disponibles.</p>
        )}
      </main>
    </div>
  )
